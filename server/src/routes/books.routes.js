// Book inventory routes — enhanced with stats endpoint
import express from "express";
import Book from "../models/Book.js";
import { authRequired } from "../middleware/auth.js";
import { requireRole } from "../middleware/roles.js";
import {
  buildDemandCalibrationOperations,
...
router.post("/", authRequired, requireRole("librarian", "staff", "admin"), async (req, res) => {
  try {
    ...
    if (!Number.isFinite(Number(payload.demandScore)) || Number(payload.demandScore) <= 0) {
      payload.demandScore = getDefaultDemandScore(payload);
    }
    payload.demandVersion = 2;
...
router.put("/:id", authRequired, requireRole("librarian", "staff", "admin"), async (req, res) => {
  try {
    const payload = { ...req.body };
    if ("demandScore" in payload && (!Number.isFinite(Number(payload.demandScore)) || Number(payload.demandScore) <= 0)) {
      payload.demandScore = getDefaultDemandScore(payload);
    }
    if ("demandScore" in payload) {
      payload.demandVersion = 2;
    }
